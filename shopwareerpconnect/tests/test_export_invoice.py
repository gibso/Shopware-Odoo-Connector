...
class TestExportInvoice(common.TransactionCase):
    """ Test the export of an invoice to Shopware """

    def setUp(self):
        super(TestExportInvoice, self).setUp()
        backend_model = self.env['shopware.backend']
        self.mag_sale_model = self.env['shopware.sale.order']
    ...
    def test_export_invoice_on_validate(self):
        ...
        with mock.patch(patched) as export_invoice:
            self._invoice_open()
            self.assertEquals(len(self.invoice.shopware_bind_ids), 1)
            export_invoice.delay.assert_called_with(
                mock.ANY,
                'shopware.account.invoice',
                self.invoice.shopware_bind_ids[0].id)
    ...
    def test_export_invoice_on_paid(self):
        ...
        with mock.patch(patched) as export_invoice:
            ...
            self.assertEquals(len(self.invoice.shopware_bind_ids), 1)
            export_invoice.delay.assert_called_with(
                mock.ANY, 'shopware.account.invoice',
                self.invoice.shopware_bind_ids[0].id)
    ...
    def test_export_invoice_api(self):
        """ Exporting an invoice: call towards the Shopware API """
        ...
        with mock_job_delay_to_direct(job_path), \
            ...
            # BackendAdapter towards Shopware to create the invoice
        ...
        self.assertEquals(len(self.invoice.shopware_bind_ids), 1)
        binding = self.invoice.shopware_bind_ids
